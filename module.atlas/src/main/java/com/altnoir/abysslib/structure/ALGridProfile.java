 ...
 * 让 per-chunk placement（{@link ALGridPlacement}）与结构类型（{@code atlas:jigsaw}）都只
 * <b>引用同一个 id</b>，从而在结构上杜绝"两边参数不一致 → 结构碎掉"这个新失败模式。
 *
 * <p>为什么需要它：原版结构类型<b>不需要</b>知道网格参数（锚点就是"传进来的那个 chunk"），
 * 所以原版只有 placement 一处参数、物理上不可能不一致。我们的 {@code atlas:jigsaw} 必须
 * 自己复算"结构中心"，于是同一组参数会出现在两个 JSON 里 —— 单一数据源就是解法。
 *
 * <p>数据包路径：{@code data/<namespace>/atlas/grid_profile/<name>.json}
 *
 * <pre>{@code

...
public final class ALStructureEnhancementCheck {
    ...
    private static void report(String phase, boolean runtimePhase) {
        ...
        try {
            ...
            AbyssLib.LOGGER.warn("[AbyssLib] 无法强制初始化 JigsawStructure，jigsaw 放宽自检可能不准：{}", t.toString());
        }
        ...
        AbyssLib.LOGGER.info(
                "[AbyssLib] 原版结构限制放宽（{}）-> jigsaw: distance={}, depth={} [codec={}, verifyRange={}, placeCommand={}]"
        ...
        if (hardFailure) {
            AbyssLib.LOGGER.warn(
                    "[AbyssLib] 有注入点未生效（通常是原版类结构变化或被其它模组覆盖）。"
    ...
    private static Boolean verifyStructureBlockNbt() {
        try {
            ...
            AbyssLib.LOGGER.warn("[AbyssLib] 结构方块 NBT 上限自检失败（不影响游戏）：{}", t.toString());
            return null;
        }
    }
}

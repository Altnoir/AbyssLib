...
/**
 * <b>一键助手</b>：把「grid profile + {@code abysslib:jigsaw} 结构 + 配套 structure_set」三条 datagen 注册
...
public final class ALStructureDatagen {
    ...
    /**
     * 结构条目：`abysslib:jigsaw` 一定要有 profile，而 bootstrap 期间只能拿到 {@code HolderGetter}
    ...
    /** 登记一个 {@code abysslib:jigsaw} 结构（不自动建 set）。 */
     ...
     * （同一 profile + {@code abysslib:per_chunk} 放置）—— 两者参数一致因此不可能写错。
     */

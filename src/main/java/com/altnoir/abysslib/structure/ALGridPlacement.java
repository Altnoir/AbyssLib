...
/**
 * <b>{@code abysslib:per_chunk}</b> —— 逐 chunk 放置：让大型结构"足迹范围内的每个 chunk 都各自持有一份结构起点"。
 *
 * <h2>它解决什么问题</h2>
 ...
 * 与结构类型 {@code abysslib:jigsaw} 引用<b>同一个</b> id → 单一数据源，不可能不一致。
...
public class ALGridPlacement extends RandomSpreadStructurePlacement {
    ...
    protected boolean isPlacementChunk(ChunkGeneratorStructureState structureState, int x, int z) {
        ...
        if (FIRST_CALL_LOGGED.compareAndSet(false, true)) {
            com.altnoir.abysslib.AbyssLib.LOGGER.debug(
                    "[AbyssLib] abysslib:per_chunk.isPlacementChunk 首次被调用：chunk=({}, {}) 命中={} (spacing={}, separation={}, footprint={})",

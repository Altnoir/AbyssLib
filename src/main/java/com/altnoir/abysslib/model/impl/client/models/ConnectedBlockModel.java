...
import com.altnoir.abysslib.model.api.client.utils.CtmState;
...
public class ConnectedBlockModel implements ALBlockModel {
    ...
    public ConnectedBlockModel(ConnectedTextureMap materials, BiPredicate<BlockState, BlockState> connectTo) {
        this(materials, connectTo, ALModelAttributes.EMPTY);
    }
    ...
    public ConnectedBlockModel(ConnectedTextureMap materials, BiPredicate<BlockState, BlockState> connectTo, RenderType renderType) {
        this(materials, connectTo, new ALModelAttributes(null, renderType));
    }

    public ConnectedBlockModel(ConnectedTextureMap materials, BiPredicate<BlockState, BlockState> connectTo, ALModelAttributes attributes) {
    ...
    public List<ALQuad> getQuads(AppearanceAndTintGetter level, BlockState state, BlockPos pos, Direction direction) {
        ...
        if (ctm.allTrue()) {
            return List.of(ALQuad.withSprite(materials.getTexture(direction, 1)));
        }

        return List.of(
                ALQuad.withState(materials, direction, ctm.up(), ctm.left(), ctm.upLeft(), 0, 0.5f, 1f, 0.5f),
                ALQuad.withState(materials, direction, ctm.up(), ctm.right(), ctm.upRight(), 0.5f, 1f, 1f, 0.5f),
                ALQuad.withState(materials, direction, ctm.down(), ctm.left(), ctm.downLeft(), 0, 0.5f, 0.5f, 0f),
                ALQuad.withState(materials, direction, ctm.down(), ctm.right(), ctm.downRight(), 0.5f, 1f, 0.5f, 0f)
        );
    }
    ...
    public Map<Direction, List<ALQuad>> getDefaultQuads(Direction direction) {
        ...
        return Map.of(
            direction,
            List.of(
                    ALQuad.withState(materials, direction, false, false, false, 0, 0.5f, 1f, 0.5f),
                    ALQuad.withState(materials, direction, false, false, false, 0.5f, 1f, 1f, 0.5f),
                    ALQuad.withState(materials, direction, false, false, false, 0, 0.5f, 0.5f, 0f),
                    ALQuad.withState(materials, direction, false, false, false, 0.5f, 1f, 0.5f, 0f)
            )
        );
    }
    ...
    @Override
    public @Nullable ALModelAttributes getAttributes() {
    ...
    private static class Factory implements ALModelFactory {
        ...
        public Supplier<ALBlockModel> create(JsonObject json) {
            ...
            var attributes = ALModelAttributes.fromJson(json);

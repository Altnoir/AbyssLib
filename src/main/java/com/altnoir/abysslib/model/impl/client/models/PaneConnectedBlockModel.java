...
public class PaneConnectedBlockModel implements ALBlockModel {

    public static final ALModelFactory FACTORY = new Factory();

    private static final List<ALQuad> CENTER = List.of(new ALQuad(1, 0, 1, 1, 0, Rotation.NONE, 0.4375f));
    ...
    public List<ALQuad> getQuads(AppearanceAndTintGetter level, BlockState state, BlockPos pos, Direction direction) {
        ...
        if (leftState && rightState) {
            ...
            return List.of(
                    new ALQuad(CtmUtils.getTexture(ctmState.up(), ctmState.left(), ctmState.upLeft()), 0, min, 1f, 0.5f, Rotation.NONE, 0.4375f),
                    new ALQuad(CtmUtils.getTexture(ctmState.up(), ctmState.right(), ctmState.upRight()), 1 - min, 1f, 1f, 0.5f, Rotation.NONE, 0.4375f),
                    new ALQuad(CtmUtils.getTexture(ctmState.down(), ctmState.left(), ctmState.downLeft()), 0, min, 0.5f, 0f, Rotation.NONE, 0.4375f),
                    new ALQuad(CtmUtils.getTexture(ctmState.down(), ctmState.right(), ctmState.downRight()), 1 - min, 1f, 0.5f, 0f, Rotation.NONE, 0.4375f)
            );
        } else if (leftState) {
    ...
    protected boolean isConnected(BlockState other, BlockState state, Direction direction) {
        if (other.is(state.getBlock())) {
            return ALModelUtils.getFromDir(other, direction.getCounterClockWise()) && ALModelUtils.getFromDir(other, direction.getClockWise());
        }
        return false;
    }

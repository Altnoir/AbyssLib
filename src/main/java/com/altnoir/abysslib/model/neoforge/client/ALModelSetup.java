...
/**
 * 内置模型加载器的客户端接线（由 {@code AbyssLibClient} 调用，专用服务端不会加载本类）。
 * <p>
 * 与上游 neoforge 模块的差异：
 * <ul>
 *     <li>上游用 {@code @Mod("athena")} 入口类；本库由 {@code AbyssLibClient} 统一调用 {@link #init}。</li>
 ...
 *     <li>几何加载器 id 由 {@code athena:athena} 改为 {@code abysslib:model}。</li>
 *     <li>新增「包裹原版模型发光」模式（本库扩展）：blockstate 根只写 {@code "abysslib:emissive": true}
...
public final class ALModelSetup {
    ...
    public static final ResourceLocation GEOMETRY_LOADER_ID =
            ResourceLocation.fromNamespaceAndPath(DefaultModels.MODID, "model");
     ...
     * <ol>
     *     <li>依次询问所有已注册的模型类型（取决于 {@code abysslib:loader}），命中者用本库模型替换；</li>
     *     <li>否则，若该方块声明了 {@code abysslib:emissive}，则把**原版模型**包一层全亮（本库扩展）；</li>
    ...
    /** 按 {@code abysslib:loader} 声明的类型接管模型；没有命中任何类型时返回 {@code null}。 */
    @Nullable
    private static BakedModel tryModelType(ModelEvent.ModifyBakingResult event, ModelResourceLocation id) {
        for (ResourceLocation type : FactoryManagerImpl.getTypes()) {
            ...
            if (model != null) {
                ...
                ALModelUtils.LOGGER.debug("AbyssLib: replaced top-level model '{}#{}' with model type {}", id.id(), id.getVariant(), type);
     ...
     * {@code "abysslib:emissive": true}、不写 {@code abysslib:loader} 时，保留 {@code variants} 指向的
    ...
    private static BakedModel wrapEmissive(ModelResourceLocation id, BakedModel original) {
        ...
        ALModelUtils.LOGGER.debug("AbyssLib: wrapped top-level model '{}#{}' with emissive (no abysslib:loader)", id.id(), id.getVariant());

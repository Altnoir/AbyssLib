...
public interface ReginthTagsProvider<T> extends ReginthLookupFillerProvider {
    ...
    interface Key<T> extends ReginthTagsProvider<T> {
        TagAppender<ResourceKey<T>, T> tag(TagKey<T> key);
    }

    interface Intrinsic<T> extends ReginthTagsProvider<T> {
        TagAppender<T, T> tag(TagKey<T> key);
    }

    class Impl<T> extends KeyTagProvider<T> implements ReginthTagsProvider.Key<T> {
        private final AbstractReginth<?> owner;
        private final ProviderType<? extends Impl<T>> type;
        private final String name;

        public Impl(AbstractReginth<?> owner, ProviderType<? extends Impl<T>> type, String name, PackOutput packOutput, ResourceKey<? extends Registry<T>> registryIn, CompletableFuture<HolderLookup.Provider> registriesLookup) {
    ...
    class IntrinsicImpl<T> extends IntrinsicHolderTagsProvider<T> implements ReginthTagsProvider.Intrinsic<T> {
        private final AbstractReginth<?> owner;
        private final ProviderType<? extends IntrinsicImpl<T>> type;
        private final String name;

        public IntrinsicImpl(AbstractReginth<?> owner, ProviderType<? extends IntrinsicImpl<T>> type, String name, PackOutput packOutput, ResourceKey<? extends Registry<T>> registryIn, CompletableFuture<HolderLookup.Provider> registriesLookup, Function<T, ResourceKey<T>> keyExtractor) {

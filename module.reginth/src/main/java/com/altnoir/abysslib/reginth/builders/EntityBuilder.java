...
public class EntityBuilder<T extends Entity, P> extends AbstractBuilder<EntityType<?>, EntityType<T>, P, EntityBuilder<T, P>> {
     ...
     */
    public static <T extends Entity, P> EntityBuilder<T, P> create(AbstractReginth<?> owner, P parent, String name, BuilderCallback callback, EntityType.EntityFactory<T> factory,
    ...
    protected EntityBuilder(AbstractReginth<?> owner, P parent, String name, BuilderCallback callback, EntityType.EntityFactory<T> factory, MobCategory classification) {
    ...
    public EntityBuilder<T, P> renderer(NonNullSupplier<NonNullFunction<EntityRendererProvider.Context, EntityRenderer<? super T, ?>>> renderer) {
        if (this.renderer == null && FMLEnvironment.getDist().isClient()) { // First call only
            ReginthDistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> this::registerRenderer);
        }
     ...
     * {@link ReginthEntityLootTables}, for creating specifically entity loot tables.
     *
     * @param cons
     *            The callback which will be invoked during entity loot table creation.
     * @return this {@link EntityBuilder}
     */
    public EntityBuilder<T, P> loot(NonNullBiConsumer<ReginthEntityLootTables, EntityType<T>> cons) {

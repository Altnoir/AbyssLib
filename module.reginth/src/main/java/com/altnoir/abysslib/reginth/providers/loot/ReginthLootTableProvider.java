...
public class ReginthLootTableProvider extends LootTableProvider implements ReginthProvider {
    ...
    public interface LootType<T extends ReginthLootTables> {
        ...
        LootType<ReginthBlockLootTables> BLOCK = register("block", LootContextParamSets.BLOCK, ReginthBlockLootTables::new);
        LootType<ReginthEntityLootTables> ENTITY = register("entity", LootContextParamSets.ENTITY, ReginthEntityLootTables::new);

        T getLootCreator(HolderLookup.Provider provider, AbstractReginth<?> parent, Consumer<T> callback);

        ContextKeySet getLootSet();

        static <T extends ReginthLootTables> LootType<T> register(String name, ContextKeySet set, TriFunction<HolderLookup.Provider, AbstractReginth<?>, Consumer<T>, T> factory) {

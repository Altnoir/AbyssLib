...
public class ReginthLootTableProvider extends LootTableProvider implements ReginthProvider {

    public interface LootType<T extends ReginthLootTables> {

        static LootType<ReginthBlockLootTables> BLOCK = register("block", LootContextParamSets.BLOCK, ReginthBlockLootTables::new);
        static LootType<ReginthEntityLootTables> ENTITY = register("entity", LootContextParamSets.ENTITY, ReginthEntityLootTables::new);

        T getLootCreator(HolderLookup.Provider provider, AbstractReginth<?> parent, Consumer<T> callback);

        LootContextParamSet getLootSet();

        static <T extends ReginthLootTables> LootType<T> register(String name, LootContextParamSet set, TriFunction<HolderLookup.Provider, AbstractReginth, Consumer<T>, T> factory) {
            LootType<T> type = new LootType<T>() {
                @Override
                public T getLootCreator(HolderLookup.Provider provider, AbstractReginth<?> parent, Consumer<T> callback) {
    ...
    private final Multimap<LootType<?>, Consumer<? super ReginthLootTables>> specialLootActions = HashMultimap.create();
    ...
    private final Set<ReginthLootTables> currentLootCreators = new HashSet<>();

    private CompletableFuture<HolderLookup.Provider> provider;

    public ReginthLootTableProvider(AbstractReginth<?> parent, PackOutput packOutput, CompletableFuture<HolderLookup.Provider> provider) {
    ...
    public <T extends ReginthLootTables> void addLootAction(LootType<T> type, NonNullConsumer<T> action) {
        this.specialLootActions.put(type, (Consumer<ReginthLootTables>) action);
    }
    ...
    private LootTableSubProvider getLootCreator(HolderLookup. Provider provider, AbstractReginth<?> parent, LootType<?> type) {
        ReginthLootTables creator = type.getLootCreator(provider, parent, cons -> specialLootActions.get(type).forEach(c -> c.accept(cons)));

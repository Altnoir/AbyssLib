...
public class ReginthBlockModelGenerator extends BlockModelGenerators {

    private final AbstractReginth<?> parent;
    public final Map<Block, BlockStateModelDispatcher> seenBlockstates = new HashMap<>();

    public ReginthBlockModelGenerator(AbstractReginth<?> parent, Consumer<BlockModelDefinitionGenerator> known, ItemModelOutput item, BiConsumer<Identifier, ModelInstance> model) {
    ...
    public ReginthLegacyBlockModelBuilder withBuilder(ExtendedModelTemplateBuilder template, TextureMapping texture) {
        return new ReginthLegacyBlockModelBuilder(modelOutput, template, texture);
    }

    public ReginthLegacyBlockModelBuilder withBuilder(ExtendedModelTemplateBuilder template) {
        return withBuilder(template, new TextureMapping());
    }

    public ReginthLegacyBlockModelBuilder getBuilder() {
        return withBuilder(new ExtendedModelTemplateBuilder());
    }

    public ReginthLegacyBlockModelBuilder withParent(ModelTemplate template) {
        return withBuilder(ExtendedModelTemplateBuilder.of(template));
    }

    public ReginthLegacyBlockModelBuilder withParent(ModelTemplate template, TextureMapping texture) {
        return withBuilder(ExtendedModelTemplateBuilder.of(template), texture);
    }

    public ReginthLegacyBlockModelBuilder withParent(TexturedModel model) {

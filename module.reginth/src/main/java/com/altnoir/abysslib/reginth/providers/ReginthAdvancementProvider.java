...
public class ReginthAdvancementProvider implements ReginthProvider, Consumer<AdvancementHolder> {
    private static final org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(ReginthAdvancementProvider.class);
    ...
    public ReginthAdvancementProvider(AbstractReginth<?> owner, PackOutput packOutputIn, CompletableFuture<HolderLookup.Provider> registriesLookupIn) {
        this.owner = owner;
    ...
    public void withConditions(@Nullable AdvancementHolder holder, List<ICondition> conditions) {
        this.registriesLookup.thenAccept(lookup -> {
            ...
            if (!seenAdvancements.add(holder.id())) {
                ...
                advancementsToSave.add(DataProvider.saveStable(cache, lookup, Advancement.CONDITIONAL_CODEC, Optional.of(new WithConditions<>(conditions, holder.value())), path));
            }
        });
    }
    ...
    public HolderLookup.@Nullable Provider getProvider() {

...
public class DataProviderInitializer {
	...
	private final Map<ProviderType<?>, ProviderType<? extends ReginthLookupFillerProvider>> providerDependencies = new ConcurrentHashMap<>();
	...
	protected List<Sorted> getSortedProviders() {
		...
		List<Map.Entry<String, ProviderType<?>>> remain = new ArrayList<>(ReginthDataProvider.TYPES.entrySet());
	...
	public void addDependency(ProviderType<?> dependent, ProviderType<? extends ReginthLookupFillerProvider> parent) {
	...
	public record Sorted(
			...
			@Nullable ProviderType<? extends ReginthLookupFillerProvider> parent
	) {
	}
